...
namespace CondoHub.Controllers
{
    ...
    public class NoticeController : Controller
    {
        ...
        public async Task<IActionResult> Solicitar()
        {
            ...
            var existe = await _context.Notices
                .AnyAsync(n => n.SolicitanteId == user.Id &&
                              (n.Status == NoticeStatus.Pendente ||
                               n.Status == NoticeStatus.EmProgresso));

            if (existe)
            {
                TempData["Error"] = "Você já possui uma solicitação em andamento!";
        ...
        [HttpPost]
        [Authorize(Roles = "MORADOR, SINDICO")]
        public async Task<IActionResult> SolicitarConfirmar()
        {
            var user = await _userManager.GetUserAsync(User);
        ...
        {
            var pendentes = await _context.Notices
                .Include(n => n.Solicitante)
                .Where(n => n.Status == NoticeStatus.Pendente)
                .ToListAsync();

            return View(pendentes);
        }

        public async Task<IActionResult> Index(string tab = "checklist")
        {
            try
            {
                var user = await _userManager.GetUserAsync(User);
                ...
                var activeTab = tab;
                if ((string.IsNullOrEmpty(tab) || tab == "checklist") && (User.IsInRole("MORADOR") || User.IsInRole("SINDICO")))
                {
                    activeTab = "historico";
                }
